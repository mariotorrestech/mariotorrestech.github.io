...
| Relative ranking system based on professional judgement ("low"/"high" and "1-5") | Fully quantitative process when all elements are quantified    |
...
- Recovery
     - Designed to recover a system or process and return to normal operation following an incident
- Compensating
...
Understanding the RMF is critical, as it informs and underpins just about every facet of operation security governance within an organization
...
| T: Tampering               | Integrity       | An attacker modifies data at rest or in transit                             |
| R: Repudiation             | Nonrepudiation  | An attacker performs an action on a system that is not attributable to them |
| I: Information Disclosure  | Confidentiality | An attacker can read sensitive information                                  |
...
| Stage                                  | Definition                                                                                                                                                                                                                                   |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1: Define Objectives                   | This considers the inherent application risk profile and addresses other business iumpact considerations early                                                                                                                               |
| 2: Define Technical Scope              | You can't protect what you don't know. This stage is intended to decompose the technology stack that supports the application components that realize the business objectives identified from Stage 1                                        |
| 3: Application Decomposition           | The stage focuses on understanding the data flows among application components and services in the application threat model                                                                                                                  |
| 4: Threat Anaysis                      | Review threat assetions from data within the environment as well as industry threat intelligence that is relevant to service, data, and deployment model                                                                                     |
...
| 6: Attack Modeling                     | This stage focuses on emulating attacks that could exploit identified weaknesses/vulnerabilites from the prior stage. It helps to also determine the threat viability via attack patterns                                                    |
...
| ------------------ | ------------------------------------------------------------------------------------ | ----- |
| D: Damage          | Total amount of damage the threat can cause?                                         | 1-10  |
| R: Reproducibility | How easily can the threat be replicated?                                             | 1-10  |
| E: Exploitability  | How difficult is it to exploit the threat?                                           | 1-10  |
| A: Affected Users  | How many people, inside or outside the organization, will be affected by the threat? | 1-10  |
| D: Discoverability | How easily can the threat be discovered?                                             | 1-10  |
